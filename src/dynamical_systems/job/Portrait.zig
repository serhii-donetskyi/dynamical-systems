...
const Factory = struct {
    fn init(allocator: Allocator, args: []const Argument) !Job {
        ...
        const t_end = args[1].value.f;
        return try Portrait.init(allocator, t_step, t_end);
    }
    fn getArguments() []const Argument {
        return &[_]Argument{
            .{ .name = "t_step", .value = .{ .f = 0.1 } },
...
test "portrait" {
    var job = try Portrait.init(std.testing.allocator, 0.1, 0.1);
    defer job.deinit();
