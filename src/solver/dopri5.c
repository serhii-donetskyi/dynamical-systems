...
static R max_of_three(R v1, R v2, R v3) {
...
static const char* step(solver_t *restrict self, const ode_t *restrict ode, R *restrict t, R *restrict x, R t_end) {
    ...
    if (h * sign <= 0 || h * sign >= h_max) h = sign * h_max;
    ...
    for (; steps < MAX_STEPS; steps++) {
        ...
        if (err < eps) {
            ...
            *t = tph;
            ...
            if (fabs(h_new) > h_max) h_new = sign * h_max;
            if (reject) h_new = fmin(fabs(h_new), h_max) * sign;
            reject = 0;
        } else {
            // Step rejected
            if (fabs(h_new) > fabs(h)) h_new = h;
...
static result_t set_data(solver_t *self, const ode_t *ode) {
    ...
    if (n <= 0) return (result_t){.type = FAILURE, .message = "ODE x_size must be positive"};
    if (self->n == n) return (result_t){.type = SUCCESS, .data = 0};
    if (self->data) solver_output.free(self->data);
    self->n = 0;
    self->data = 0;
    R* data = (R*)solver_output.malloc(size);
    if (!data) return (result_t){.type = FAILURE, .message = "Failed to allocate memory"};
...
static result_t create(const argument_t *args) {
    ...
    if (h_max <= 0 || h_max >= 1) return (result_t){.type = FAILURE, .message = "h_max must satisfy: 0 < h_max < 1"};
    if (eps <= 0 || eps >= 1) return (result_t){.type = FAILURE, .message = "eps must satisfy: 0 < eps < 1"};
    ...
    if (!solver || !solver_args) {
        if (solver) solver_output.free(solver);
        if (solver_args) solver_output.free(solver_args);
...
static void destroy(solver_t *solver) {
    ...
    if (solver->data) solver_output.free(solver->data);
